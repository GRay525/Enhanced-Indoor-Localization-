...
'''
x=np.arange(-2,4,2)
y=[np.sqrt(4-e*e) for e in x]
plt.plot(x,y,'*')
plt.show() # get
X1=np.linspace(-4,4,100,endpoint=True)
plt.plot(X1,(X1**2)/9) # get paowuxian
...
for i in range(4):
    sensor[i,:]=[x[i],y[i]]
    point.append(Point(x[i], y[i]))
for i in range(4):
    for o in range(2):
      c.append(point[o].buffer(b[o]))
      cir_list.append(c[o])


a1 = c[0].intersects(c[1])
inter = a1
area = inter.area
from matplotlib.collections import PatchCollection
from matplotlib.patches import Polygon
from shapely.geometry import LineString
# plot the circle before and after merging

patches = PatchCollection([Polygon(a.exterior) for a in cir_list], facecolor='red', linewidth=.5, alpha=.5)
intersect_patch = PatchCollection([Polygon(inter.exterior)], facecolor='red', linewidth=.5, alpha=.5)
# make figure
...
def for_test_generate_simu_data():
    ...
    xyz_seat = np.c_[xy_seat, np.ones((len(xy_seat), 1)) * z_ap] #xy_seat combine with (a all one line matrix * z_ap) : 2 demonxion targets
    # xyz_anchor_1 = np.c_[xy_anchor, np.ones((len(xy_anchor), 1)) * 0.] # anchor 2 demonsion set their z is 0
    # xyz_anchor_2 = np.c_[xy_anchor, np.ones((len(xy_anchor), 1)) * 2.0]# anchor 2 demonsion set their z is 2
    # xyz_anchor = np.row_stack((xyz_anchor_1,xyz_anchor_2))
    _X, _Y, _Z = np.meshgrid([0, 2.36, 4.72], [0, 0.7 * 51], [0, 2])
    xyz_anchor = np.c_[_X.flat, _Y.flat, _Z.flat]
    x_val = [0, 2.36, 4.72],
    y_val = [7 * i for i in range(6)]
    _X, _Y = np.meshgrid(x_val, y_val)
    xyz_3d_target = np.c_[_X.flat, _Y.flat, np.ones((_X.size, 1)) * 1.5] # set 3d target z is 1.5
    ...
    d_true_2d = np.sqrt(
        np.sum((xy_devices.reshape(-1, 1, 2) - xy_devices.reshape(1, -1, 2)) ** 2, axis=2)
    )
    d_true[d_true == 0] = np.nan
    d_true_2d[d_true_2d == 0] = np.nan
    rss_true = p0 - gamma * 10 * np.log10(d_true / d0)  # actual rss
    rss_true_2d = p0 - gamma * 10 * np.log10(d_true_2d / d0)
...
def joint_estimation(rss_meas, xyz_anchor,
    ...
    kmin = []
    index_min = {}
    index_amin = [i for i in range(len(xyz_target)-1)]
    for i_iter in range(max_iter):
        ...
        A = np.argsort(np.abs(xyz_target[n2:-1,:]), axis=0)
        num_min_v = int((n3) * 0.02)
        kmin = A[0:num_min_v, 2]  # 1st time get 5%; second time get 5%+5%
        num_min_a.append(num_min_v)

        index_m = []

        for j in kmin:
           index_m.append(j)
           index_min[i_iter] = index_m
        length_min = len(kmin)

        d = []
        rss_mm_c1 = rss_meas_c
        rss_mm1 = rss_meas
        b = np.zeros((1, len(rss_meas_c)))
        c = np.zeros((1, len(rss_meas_c)))
        for i in range(len(kmin)):
           index_amin.insert(0, kmin[len(kmin) - i - 1] + i_iter * num_min_v)
           index_amin.pop(kmin[len(kmin) - i - 1] + 1 + i_iter * num_min_v)




        for i in range(len(kmin)):
              d.append(xyz_target[kmin[i], :])
        a = d[::-1]
        if length_min != 0:
          for i in range(length_min):
                rss_mm1 = np.r_[b, rss_mm1]
                rss_mm1[0, :] = rss_mm1[kmin[i] + 1, :]
                rss_mm1 = np.delete(rss_mm1, kmin[i] + 1, axis=0)
                rss_mm1 = np.c_[c.T, rss_mm1]
                rss_mm1[:, 0] = rss_mm1[:, kmin[i] + 1]
                rss_mm1 = np.delete(rss_mm1, kmin[i] + 1, axis=1)
                rss_mm_c1 = np.r_[b, rss_mm_c1]
                rss_mm_c1[0, :] = rss_mm_c1[kmin[i] + 1, :]
                rss_mm_c1 = np.delete(rss_mm_c1, kmin[i] + 1, axis=0)
                rss_mm_c1 = np.c_[c.T, rss_mm_c1]
                rss_mm_c1[:, 0] = rss_mm_c1[:, kmin[i] + 1]
                rss_mm_c1 = np.delete(rss_mm_c1, kmin[i] + 1, axis=1)
          rss_mt1 = rss_mm1
          rss_mtc1 = rss_mm_c1
        else:
            rss_mt1 = rss_meas
        ...
        xyz_t = np.delete(xyz_target, kmin, axis=0)


        a = np.array(a)

        if len(a) != 0:
          xyz_t1 = np.concatenate((a, xyz_t),axis=0)
          xyz_target = xyz_t1
    ...
    return xyz_target, gamma, p0, g_tx, g_rx, index_amin, num_min_a
...
def one_test(i, sigma2_rss_meas):
    ...
    try:
        xyz_target_est, gamma_est, p0_est, g_tx_est, g_rx_est, index = \
...
def test_overall_algorithm():
    ...
    xyz_seat, xyz_3d_target, xyz_anchor, z_ap, d_true, gamma, p0, d0, rss_true = for_test_generate_simu_data()
    n2, n3, m = 0, len(xyz_seat) + len(xyz_3d_target), len(xyz_anchor)
    g_tx = np.random.randn(len(rss_true))
    g_rx = np.random.randn(len(rss_true))
    e = np.random.randn(rss_true.size) * np.sqrt(sigma2_rss_meas)

    rss_meas = rss_true #+ e.reshape(*rss_true.shape)\
                    #+ g_tx.reshape(1, -1) + g_rx.reshape(-1, 1) \

    index = []
    xyz_seat_possible = xyz_seat
    xyz_3d_possible = xyz_3d_target
    xyz_target_est, gamma_est, p0_est, g_tx_est, g_rx_est, index, num_min_a = \
        joint_estimation(rss_meas, xyz_anchor,
                         z_ap,index,
                         n2=n2, n3=n3,
                         max_iter=2,
    ...
    num_min = sum(num_min_a)
    nn = len(xyz_seat)
    xyz_seat_t = xyz_seat
    xyz_seat_possible_t = xyz_seat_possible
    xyz_3d_possible_t = xyz_3d_possible
    xyz_3d_target_t = xyz_3d_target
    a1 = np.array([])
    a11 = np.array([])
    a2 = np.array([])
    a22 = np.array([])
    for j in index[0: num_min]:

         if j < nn:
             a1 = np.append(a1, [xyz_seat_t[j, :]])
             a11 = np.append(a11, [xyz_seat_possible_t[j, :]])

         else:
             a2 = np.append(a1, xyz_3d_target_t[j - nn, :])
             a22 = np.append(a11, xyz_3d_possible_t[j - nn, :])

    a1 = np.reshape(a1, (-1, 3))
    a11 = np.reshape(a11, (-1, 3))
    a2 = np.reshape(a2, (-1, 3))
    a22 = np.reshape(a22, (-1, 3))

    if len(a1) != 0:
        xyz_seat_t = np.delete(xyz_seat_t, index[0: num_min], axis=0)
        xyz_seat_t = np.vstack((a1, xyz_seat_t))
    else:

        xyz_seat_t = xyz_seat_t
    if len(a11) != 0:
        xyz_seat_possible_t = np.delete(xyz_seat_possible_t, index[0: num_min], axis=0)
        xyz_seat_possible_t = np.vstack((a11, xyz_seat_possible_t))
    else:
        xyz_seat_possible_t = xyz_seat_possible_t
    if len(a2) != 0:
        xyz_3d_target_t = np.delete(xyz_3d_target_t, [i - nn for i in index[0: num_min]], axis=0)
        xyz_3d_target_t = np.vstack((a2, xyz_3d_target_t))
    else:
        xyz_3d_target_t = xyz_3d_target_t
    if len(a22) != 0:
        xyz_3d_possible_t = np.delete(xyz_3d_possible_t, [i - nn for i in index[0: num_min]], axis=0)
        xyz_3d_possible_t = np.vstack((a22, xyz_3d_possible_t))
    else:
        xyz_3d_possible_t = xyz_3d_possible_t




    is_correct_3d = do_association(xyz_target_possible=np.row_stack((xyz_seat_possible_t, xyz_3d_possible_t)),
                                   xyz_target_est=xyz_target_est,
                                   xyz_target_true=np.row_stack((xyz_seat_t, xyz_3d_target_t)))
    print('accuracy of 3D localization: ', sum(is_correct_3d) / len(is_correct_3d))
    do_plot(xyz_target_est[:n2], xyz_target_est[n2:n2 + n3], xyz_anchor)
    x=1
